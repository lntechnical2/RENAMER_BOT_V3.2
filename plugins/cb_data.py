...
async def vid(bot,update):
     ...
     if value < file.file_size:
         await ms.edit("```Trying To Upload```")
         try:
             filw = await app.send_video(log_channel,document = file_path,thumb=ph_path,duration=duration ,caption = f"**{new_filename}**",progress=progress_for_pyrogram,progress_args=( "```Trying To Uploading```",  ms, c_time   ))
             from_chat = filw.chat.id
             mg_id = filw.id
             time.sleep(2)
             await bot.copy_message(update.from_user.id,from_chat,mg_id)
     		...
     		c_time = time.time()
     		try:
     			await bot.send_video(update.from_user.id,document = file_path,thumb=ph_path,duration=duration,caption = f"**{new_filename}**",progress=progress_for_pyrogram,progress_args=( "```Trying To Uploading```",  ms, c_time   ))			
     			await ms.delete()
     			os.remove(file_path)
     		except Exception as e:
     			neg_used = used - int(file.file_size)
